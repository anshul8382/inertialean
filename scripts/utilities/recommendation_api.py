...
class AssetAllocationAPI:
    ...
    def calculate_asset_allocation(
        ...
        try:
            ...
            # 2. Calculate target portfolio value
            target_portfolio_value = total_portfolio_value + investment_amount
            
            # 3. Get asset allocation model for client
            model_assignment = self._get_client_models(client_id)
            asset_allocation_model = model_assignment.asset_model
            
            # 4. Calculate target allocation for each asset class
            asset_allocations = []
            for allocation in asset_allocation_model.asset_allocations:
                ...
                # Calculate target value for this asset class
                target_value = (float(target_weight) / 100.0) * target_portfolio_value
                current_value = current_state.get('current_asset_allocations', {}).get(asset_class.name, 0)
                ...
                # Apply asset class-specific thresholds
                allocation_action = self._determine_allocation_action(
                    asset_class.name, 
                    current_value, 
                    target_value, 
                    required_change
    ...
    def _get_client_models(self, client_id: int):
        """Get model assignment for a client"""
        try:
            from models import Client, ModelAssignment
            client = Client.query.get_or_404(client_id)
            return ModelAssignment.query.filter_by(client_id=client.id).order_by(
                ModelAssignment.assigned_at.desc()
            ).first()
        except Exception as e:
            self.logger.error(f"Error getting models for client {client_id}: {e}")
            return None
            ...
            for allocation in equity_model.security_allocations:
                ...
                if quantity_change != 0:
                    recommendations.append({
                        ...
                        'quantity': int(abs(int(quantity_change))),
    ...
    def calculate_security_recommendations(self, client_id, asset_class, required_change, target_value, current_asset_value):
        ...
        try:
            ...
            for allocation in fixed_income_model.security_allocations:
                ...
                if required_change > 0:  # Need to buy more Fixed Income
                    ...
                    action = 'BUY'
                else:  # Need to sell Fixed Income
...
class SecurityAllocationAPI:
    ...
    def generate_security_recommendations(
        ...
        try:
            ...
            return {
                ...
                'target_value': target_value,
                'security_recommendations': result.get('security_recommendations', {}).get(asset_class, []),
                'total_recommendations': len(result.get('security_recommendations', {}).get(asset_class, []))
            }
    ...
    def calculate_security_allocations(
        ...
        try:
            ...
            self.logger.error(f"Error in Stage 2 (Security Allocation): {e}")
            return {'error': str(e)}
